...
class Agent
{
		...
		while(($row = fgetcsv($file, null, ';')) !== false) {
			...
			$this->loadLocation(
				...
				[
					...
					'ORIG_NAME'       => $origName = implode(', ', [trim($country), $region, trim($cityName)]),
	...
	protected function loadLocation($city, $additFields = array())
	{
		...
		if (!empty($fields['SUBREGION_NAME'])) {
			$fields['REGION_NAME'] .= ', '. $fields['SUBREGION_NAME'];
		}
		unset($fields['SUBREGION_NAME']);
		...
		if ($exists) {
			unset($fields['ORIG_NAME']); unset($fields['ORIG_NAME_LOWER']); unset($fields['REGION_NAME']);
