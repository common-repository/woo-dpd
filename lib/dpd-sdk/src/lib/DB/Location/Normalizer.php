...
{      
    ...
    public function normilizeRegion($region, $country)
    {
        ...
        if ($this->trimAbbr($data[0], $this->getSubregionAbbrList())) {
            $subregion = $data[0];
            $region    = $data[1];
        } elseif ($this->trimAbbr($data[1], $this->getSubregionAbbrList())) {
            $subregion = $data[1];
            $region    = $data[0];
        }
    ...
    protected function getSubregionAbbrList()
    {
        return [
            'район',
            'р-н',
